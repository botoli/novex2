...
try {
  ...
  if (env.parsed && env.parsed.VITE_API_URL) {
    console.log(`   ✅ VITE_API_URL в .env.production: ${env.parsed.VITE_API_URL}`);
    if (env.parsed.VITE_API_URL.includes('localhost') || env.parsed.VITE_API_URL.includes('127.0.0.1')) {
...
(async () => {
  ...
  const req = http.request(corsUrl, { method: 'OPTIONS' }, (res) => {
    ...
    if (corsHeader) {
      ...
      if (corsHeader === '*' || corsHeader.includes('novextask.ru')) {
        ...
        console.log(`   ⚠️  Возможно, домен novextask.ru не разрешен: ${corsHeader}`);
      }
    } else {
  ...
  try {
    const buildCheck = execSync('grep -r "localhost\\|127.0.0.1\\|:8000\\|:8080" frontend/dist/ 2>/dev/null | head -5', { encoding: 'utf8' });
    if (buildCheck.trim()) {
      console.log('   ⚠️  В собранных файлах найдены локальные адреса:');
      console.log(buildCheck.split('\n').map(line => `      ${line}`).join('\n'));
    } else {
  ...
  if (allPassed) {
    ...
    console.log('1. Убедитесь, что в Vercel добавлена переменная VITE_API_URL=https://api.novextask.ru');
    console.log('2. Настройте DNS запись A для api.novextask.ru на IP 94.29.6.121');
